...
class FullyConnectedNet(object):
    ...
    Network contains an arbitrary number of hidden layers, ReLU non linearities,
    ...
    def __init__(
        ...
        for i in range(1, self.num_layers + 1):
            w, b = 'W' + str(i), 'b' + str(i)
            gamma, beta = "gamma" + str(i), "beta" + str(i)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            if self.normalization == "batchnorm":
                gamma, beta = "gamma" + str(i), "beta" + str(i)
                out, cache = batchnorm_forward(out, self.params[gamma], self.params[beta], self.bn_params[i - 1])
                cache_norm.append(cache)
            # Layer Norm
            if self.normalization == "layernorm":
                gamma, beta = "gamma" + str(i), "beta" + str(i)
                out, cache = layernorm_forward(out, self.params[gamma], self.params[beta], self.bn_params[i - 1])
                cache_norm.append(cache)
            # Relu
        ...
        dout, grads[w], grads[b] = affine_backward(dx, cache_affine.pop())
        # Add the regularization items
        ...
        while i >= 0:
            # Drop out
            if self.use_dropout:
                dout = dropout_backward(dout, cache_drop.pop())
            # ReLU
            dout = relu_backward(dout, cache_relu.pop())
            # Batch Norm
            if self.normalization == 'batchnorm':
                dout, grads['gamma' + str(i+1)], grads['beta' + str(i+1)] = batchnorm_backward(dout, cache_norm.pop())
            # Layer Norm
            if self.normalization == 'layernorm':
                dout, grads['gamma' + str(i+1)], grads['beta' + str(i+1)] = layernorm_backward(dout, cache_norm.pop())
            w = 'W' + str(i+1)
            b = 'b' + str(i+1)
            dout, grads[w], grads[b] = affine_backward(dout, cache_affine.pop())
