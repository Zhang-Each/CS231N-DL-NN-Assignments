...
def softmax_loss(x, y):
    ...
    f = np.exp(x)
    softmax = f / np.sum(f, axis=-1).reshape((f.shape[0], 1))
    loss = -np.log(softmax[np.arange(softmax.shape[0]), y])
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        # running_mean 和 running_value 是在训练时候累加的，用在测试阶段的计算
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
        new_x = (x - sample_mean) / np.sqrt(sample_var + eps)
        out = gamma * new_x + beta
        cache = (x, gamma, beta, new_x, sample_mean, sample_var, eps)
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
        ...
        new_x = (x - running_mean) / np.sqrt(running_var + eps)
        out = gamma * new_x + beta
...
def batchnorm_backward_alt(dout, cache):
    ...
    as batch norm_backward, but might not use all of the values in the cache.

    Inputs / outputs: Same as batch norm_backward
    """
    dx, dgamma, dbeta = None, None, None
    ###########################################################################
    # TODO: Implement the backward pass for batch normalization. Store the    #
    ...
    dx = 1 / np.sqrt(var + eps) * dx_hat + dvar * 2 * (x - mu) / m + dmu / m
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    # 在层标准化的情况下，我们需要对mean和variance的计算结果进行reshape
    sample_mean = np.mean(x, axis=-1).reshape(x.shape[0], -1)
    sample_var = np.var(x, axis=-1).reshape(x.shape[0], -1)
    new_x = (x - sample_mean) / np.sqrt(sample_var + eps)
    out = gamma * new_x + beta
    cache = (x, gamma, beta, new_x, sample_mean, sample_var, eps)
...
def layernorm_backward(dout, cache):
    ...
    dx_hat = dout * gamma
    d_var = np.sum(dx_hat * (x - mu) * -0.5 * ((var + eps) ** -1.5), axis=1)
    d_mean = (np.sum(dx_hat * -1 / np.sqrt(var + eps), axis=1) + d_var * np.mean(-2 * (x - mu), axis=1))
    dx = 1 / np.sqrt(var + eps) * dx_hat + 2 * d_var.reshape(-1, 1) / M * (x - mu) + 1 / M * d_mean.reshape(-1, 1)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(N):
        ...
        for j in range(F):
            kernel, bias = w[j], b[j]
            # 开始对一个(C, HHH, WWW)的图片和(C, HH, WW)的卷积核进行卷积操作
            for h in range(HHH):
                for ww in range(WWW):
                    conv_patch = pixels[:, h * stride: h * stride + HH, ww * stride: ww * stride + WW]
                    conv = np.sum(conv_patch * kernel) + bias
                    out[i, j, h, ww] = conv
        ...
        for j in range(F):
            ...
            for hh in range(H_prime):
                for ww in range(W_prime):
                    ...
                    db[j] += dout[i, j, hh, ww]
                    dx_pad[
                        i, :, hh * stride: hh * stride + HH, ww * stride: ww * stride + WW
                    ] += dout[i, j, hh, ww] * w_j
                    dw[j] += dout[i, j, hh, ww] * x_i[:, hh * stride: hh * stride + HH, ww * stride: ww * stride + WW]
...
def max_pool_forward_naive(x, pool_param):
    ...
    HH, WW = int(1 + (H - h) / stride), int(1 + (W - w) / stride)
