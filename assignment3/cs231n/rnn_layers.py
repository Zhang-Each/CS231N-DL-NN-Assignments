...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    and the mini batch is of size N.

    Inputs:
    - x: Input data for this time step, of shape (N, D)
    - prev_h: Hidden state from previous time step, of shape (N, H)
...
def rnn_step_backward(dnext_h, cache):
    ...
    df = (1 - next_h ** 2) * dnext_h
    dx = np.matmul(df, Wx.T)
    db = np.sum(df, axis=0)
    dprev_h = np.matmul(df, Wh.T)
    dWx = np.matmul(x.T, df)
    dWh = np.matmul(prev_h.T, df)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h = np.zeros((N, T, H))
    for i in range(T):
        prev_h, cache_step = rnn_step_forward(x[:, i, :].reshape(N, D), prev_h, Wx, Wh, b)
        h[:, i, :] = prev_h
...
def word_embedding_forward(x, W):
    ...
    We operate on mini batches of size N where
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        ...
        next_h, next_c, cache_step = lstm_step_forward(x_step, next_h, next_c, Wx, Wh, b)
        h[:, i, :] = next_h
        cache[i] = cache_step
    cache['size'] = (N, T, D)
...
def lstm_backward(dh, cache):
    ...
    for i in range(T - 1, -1, -1):
        dh_prev += dh[:, i, :]
        dx_step, dh_prev, dc_prev, dwx, dwh, db_step = lstm_step_backward(dh_prev, dc_prev, cache[i])
        dx[:, i, :] = dx_step
        dh0 = dh_prev
        dWx += dwx
        dWh += dwh
