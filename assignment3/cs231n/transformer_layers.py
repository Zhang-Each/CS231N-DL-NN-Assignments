...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pe = torch.zeros(1, max_len, embed_dim)  # 1 * l * d
        ############################################################################
        ...
        # this is what the auto grader is expecting. For reference, our solution is#
        ...
        for i in range(max_len):
            for j in range(embed_dim):
                if j % 2 == 0:
                    pe[:, i, j] = torch.sin(torch.tensor(i * 10000 ** (- j / embed_dim)))
                else:
                    pe[:, i, j] = torch.cos(torch.tensor(i * 10000 ** (-(j - 1) / embed_dim)))
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.1):
        ...
        # outputs relative to the auto grader). Note that the layers use a bias
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        ############################################################################
        # TODO: Implement multi headed attention using the equations given in      #
        ...
        H = self.heads
        E = D // H
        # 此时q变成N*H*S*E,k变成N*H*T*E,v变成N*H*T*E
        q = self.query(query).reshape(N, S, H, E).permute(0, 2, 1, 3)
        k = self.key(key).reshape(N, T, H, E).permute(0, 2, 1, 3)
        v = self.value(key).reshape(N, T, H, E).permute(0, 2, 1, 3)
        # 这里的k变成了N*H*E*T，得到的结果应该是N*H*S*T
